...
Ext.define('Ext.ux.IFrame', {
	...
	renderTpl: [
		...
		'<div class="x-empty-iframe" id="empty-{frameName}" style="width:100%;height:100%">{emptyData}</div>'
	],
	...
	initComponent: function () {
		...
		Ext.apply(this.renderSelectors, {
			iframeEl: 'iframe',
			emptyEl: 'div.x-empty-iframe'
		});

		this.callParent()

		this.on('afterrender', this.display, this);
	},
	...
	emptySrc: function(){
		return !this.src || this.src == '' || this.src == 'about:blank';
	},
	...
	display: function(){
		...
		if(this.emptySrc()){
			...
			this.emptyEl.addCls('x-hidden');
			this.iframeEl.removeCls('x-hidden');
		}
	},

	empty: function(tpl){
		if(tpl) {
			this.emptyEl.update(tpl);
		};

		this.emptyEl.removeCls('x-hidden');
		this.iframeEl.addCls('x-hidden');
	},
