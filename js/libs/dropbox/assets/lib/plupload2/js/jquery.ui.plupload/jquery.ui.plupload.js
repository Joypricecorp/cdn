...
var lastActive, startXPos, startYPos, clickDragged,
	...
	formResetHandler = function() {
		var form = $( this );
		setTimeout(function() {
			form.find( ":ui-button" ).jpxbutton( "refresh" );
		}, 1 );
	},
		...
		if ( this.type === "checkbox" ) {
			...
			this.buttonElement.bind( "click" + this.eventNamespace, function() {
				...
				radioGroup( radio )
					.not( radio )
					.map(function() {
						return $( this ).jpxbutton( "widget" )[ 0 ];
					})
...
$.widget( "ui.buttonset", {
	...
	_setOption: function( key, value ) {
		if ( key === "disabled" ) {
			this.buttons.jpxbutton( "option", key, value );
		}
...
$.widget("ui.plupload", {
	...
	_create: function() {
		...
		if ($.ui.jpxbutton) {
			this.browse_button.jpxbutton({
				icons: { primary: 'ui-icon-circle-plus' },
				disabled: true
			});
			
			this.start_button.jpxbutton({
				icons: { primary: 'ui-icon-circle-arrow-e' },
				disabled: true
			});
			
			this.stop_button.jpxbutton({
	...
	_initUploader: function() {
		...
		uploader.bind('PostInit', function(up) {	
			...
			if (!self.options.buttons.browse) {
				self.browse_button.jpxbutton('disable').hide();
				up.disableBrowse(true);
			} else {
				self.browse_button.jpxbutton('enable');
			}
			
			if (!self.options.buttons.start) {
				self.start_button.jpxbutton('disable').hide();
			} 
			
			if (!self.options.buttons.stop) {
				self.stop_button.jpxbutton('disable').hide();
			}
			...
			self.start_button.click(function(e) {
				if (!$(this).jpxbutton('option', 'disabled')) {
					self.start();
				}
				e.preventDefault();
			});
	...
	_setOption: function(key, value) {
		...
		if (key == 'buttons' && typeof(value) == 'object') {	
			...
			if (!value.browse) {
				...
				self.browse_button.jpxbutton('enable').show();
			...
			if (!value.start) {
				...
				self.start_button.jpxbutton('enable').show();
			}
	...
	*/
	enable: function() {
		this.browse_button.jpxbutton('enable');
	...
	*/
	disable: function() {
		this.browse_button.jpxbutton('disable');
	...
	*/
	...
	_handleState: function() {
		...
		if (up.state === plupload.STARTED) {
			$(self.start_button).jpxbutton('disable');
			...
			if (self.options.multiple_queues) {
				$(self.start_button).jpxbutton('enable');
	...
	_updateTotalProgress: function() {
		...
		if (up.files.length === (up.total.uploaded + up.total.failed)) {
			this.start_button.jpxbutton('disable');
		} else {
			this.start_button.jpxbutton('enable');
		}
	...
	_enableViewSwitcher: function() {
		...
		if (buttons.length === 1) {
			...
			return;
		} else if ($.ui.jpxbutton && buttons.length > 1) {
			switcher.show();
			switcher.buttonset();
		} else {
			switcher.show();
